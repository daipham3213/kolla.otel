...
class InstrumentCommandTestCase:
    """The ``otel-instrument`` command (shared ``_OtelCommand`` behaviour)."""

    def test_get_parser_exposes_config_and_ansible_args(
        self, config_file
    ) -> None:
    ...
    def test_get_parser_config_is_optional(self) -> None:
        """--config may be omitted (config then comes from globals.yml)."""
        args = _command().get_parser("instrument").parse_args([])
        assert args.config is None

    def test_take_action_runs_playbook_without_config(self, recorder) -> None:
    ...
    def test_take_action_translates_config_to_extra_vars(
        ...
        names = [
            s["container_name"] for s in extra["otel_instrument_services"]
        ]
        assert names == ["nova_api", "cinder_api"]

    def test_take_action_invalid_config_raises(
    ...
    def test_load_document_reads_yaml(self, config_file) -> None:
    ...
    def test_load_document_missing_file_raises(self, tmp_path) -> None:
        """A missing file surfaces a domain error."""
        with pytest.raises(KollaOtelError):
            Instrument._load_document(tmp_path / "nope.yml")

    def test_load_document_invalid_yaml_raises(self, tmp_path) -> None:
...
class RollbackCommandTestCase:
    """The ``otel-rollback`` command."""

    def test_runs_rollback_playbook_without_config(self, recorder) -> None:
    ...
    def test_translates_config_to_extra_vars(
        self, recorder, config_file
    ) -> None:
